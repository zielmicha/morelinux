...
proc readResponse*(sock: SocketHandle, bulk=false, ignoreError=false): seq[string] =
  ...
  while not stop:
    ...
    while i + sizeof(nlmsghdr) <= v:
      ...
      if response.kind == NLMSG_ERROR and not ignoreError:
        raise newException(ValueError, "Netlink returned error (err: $1)" % repr(result[^1]))
...
proc makeMessage*(kind: uint16, bulk: bool, body: string): string =
  var hdr: nlmsghdr
  hdr.len = sizeof(nlmsghdr).uint32 + body.len.uint32
  hdr.kind = kind
  if bulk:
    hdr.flags = NLM_F_MATCH or NLM_F_ROOT or NLM_F_REQUEST
  else:
    hdr.flags = NLM_F_REQUEST or NLM_F_ACK
  hdr.seqNum = 0
  hdr.pid = 0
...
proc newNetlinkSocket*(kind: cint): SocketHandle =
  let sock = socket(AF_NETLINK, SOCK_DGRAM, 0)
  if sock.int < 0:
    raiseOSError(osLastError())
...
proc sendMessage*(sock: SocketHandle, msg: string) =
